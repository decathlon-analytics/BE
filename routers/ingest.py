...
from services.ingest import ingest_reviews_df, ingest_summary_df, ingest_product_info_df
...
@router.post("/product-info/file")
async def ingest_product_info_file(file: UploadFile = File(...)):
    df = pd.read_csv(io.BytesIO(await file.read()))
    return ingest_product_info_df(df)
...
def ingest_product_info(csv_path: str):
    if not os.path.exists(csv_path):
        raise HTTPException(400, f"CSV not found: {csv_path}")
    return ingest_product_info_df(pd.read_csv(csv_path))